...
2026-05-09 在 `puersai-hpc` 上完成了真实图片集成测试。该机器更接近目标生产环境，应作为后续性能验证和回归测试的首选环境。环境特征如下：
...
同一批 149 张真实图片在 Windows D 盘挂载路径和 WSL 原生路径下的对比结果如下：

| 测试路径 | 文件数 | 扫描吞吐 | 修图处理吞吐 | 处理失败 | 质量发现 |
| --- | ---: | ---: | ---: | ---: | ---: |
| `/mnt/d/test-pic-native-compare` | 149 | 32.47 张/分钟 | 25.31 张/分钟 | 0 | 24 个，P0=22，P1=0，P2=2 |
| `/home/ps/test-pic-private` | 149 | 32.25 张/分钟 | 25.22 张/分钟 | 0 | 24 个，P0=22，P1=0，P2=2 |

结论：

- WSL 访问 D 盘挂载路径不是当前主要瓶颈。同批次、同参数测试下，两种路径吞吐差异低于 1%。
- 当前瓶颈主要来自 CPU 图像解码、Pillow 图像处理、重复读写派生图、规则计算与进程调度。
- 当前程序属于可用原型，不满足推荐硬件下 600 张/分钟以上的生产目标。后续代码工作必须先围绕处理引擎和流水线做性能优化。
- GPU 可见但未被当前程序使用。这是当前程序能力限制，不是硬件环境限制。GPU 应先用于模型深检、OCR、缺陷检测、批量张量推理等收益明确的任务，不应盲目替代全部传统图像处理。

后续所有性能和真实图片回归测试默认在 `puersai-hpc` 执行。真实图片不得上传到公网、GitHub、Linear 或外部服务；对外只允许提交聚合指标、隐私自检结果和不含文件名/路径/哈希/图像内容的摘要。
...
| 优先级 | 优化方向 | 设计要求 | 验收方式 |
| --- | --- | --- | --- |
| P0 | 性能观测与基准稳定化 | 增加阶段级计时、文件数、吞吐、P50/P95、CPU/内存/磁盘/GPU 利用率采集；固定 `puersai-hpc` 测试脚本和聚合报告格式。 | 同一批 149 张真实图片可一键跑出可比较的 baseline JSON；报告不含隐私明细。 |
| P0 | 避免重复解码和重复写盘 | 扫描、规则检测、处理计划和修图尽量复用一次解码结果；无变化或低收益处理不重写派生图；重复文件复用检测结果。 | 处理失败为 0；吞吐较当前 25 张/分钟有可测提升；处理审计仍可回溯。 |
| P0 | OpenCV 后端替换热点算法 | 将 deskew、边缘/内容框检测、去孤立点等 CPU 热点从 Pillow/Python 循环迁移到 OpenCV/NumPy 向量化实现。 | 同批真实图片质量发现数量与当前基线一致或差异可解释；处理吞吐提升。 |
...
- 默认测试环境为 `puersai-hpc`；除单元测试外，真实图片集成测试不得在开发机或 CI 上替代。
- 不上传真实图片、文件名、路径、哈希、缩略图、OCR 文本或 row-level 缺陷明细。
- PR 只提交代码、文档和聚合验证结果；敏感本地报告留在 `puersai-hpc`。
...
- 性能指标：短期以 `puersai-hpc` 当前 149 张真实图片基线为起点，扫描 32 张/分钟、修图处理 25 张/分钟不得退化；阶段性优化目标先达到 2 倍、5 倍、10 倍提升。生产目标是在推荐配置下标准质检修图达到 600 张/分钟以上，AI 深检达到 120 张/分钟以上。
