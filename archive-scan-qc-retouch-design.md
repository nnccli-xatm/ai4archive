...
目标平台：先完成本地单机生产能力；局域网集中处理场景放到本地能力成熟后实现；统信 UOS、银河麒麟、openKylin、aarch64、LoongArch 等国产化适配暂时搁置，待明确通知后再启动。
...
UI 建议采用 Web 工作台，控制面直接提供静态前端和 REST API。近期重点是让本地浏览器生产工作台封装批量质检、自动安全修图、问题复核和处理副本输出；后续再评估国产桌面系统上的浏览器壳或离线包。
