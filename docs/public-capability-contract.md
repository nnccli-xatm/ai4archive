...
`ocr_preprocess_stroke_bg`, `ocr_preprocess_structure`,
`ocr_preprocess_deskew_clarity`,
`standard-conservative-v1`, `ocr-preprocess-leptonica-v1`,
`ocr-preprocess-opencv-local-v1`, `ocr-preprocess-sauvola-wolf-v1`, or
`ocr-preprocess-stroke-bg-v1`, `ocr-preprocess-structure-v1`, or
...
sidecar route, a stroke-protected background normalization route, or a
structure-preserving OCR route for text/table-line preservation, or a
deskew-clarity OCR route for comparing preserve-canvas rotation candidates, but they do not expose local rows, paths, filenames, hashes, OCR
