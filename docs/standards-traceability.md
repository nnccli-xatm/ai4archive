...
| DA/T 31-2017 10.2 rotation and deskew | Pages that require rotation or skew correction should be processed consistently and then reviewed. | `batch_orientation_consistency`; optional processing manifest records EXIF transpose and deskew decisions. | Automated orientation screening plus opt-in derivative deskew metadata. Findings are review prompts, not final manual acceptance decisions. | Add sampled human review instructions for orientation-heavy collections. |
| DA/T 31-2017 10.3 border cropping | Border cropping should be controlled so page information is not removed. | Optional processing manifest records crop and dark-border trim decisions, output size, and source hash. | Implemented as opt-in local derivative processing; original files are not modified. | Add operator approval workflow for derivative use in production packages. |
