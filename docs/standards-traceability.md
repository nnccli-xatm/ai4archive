...
| DA/T 31-2017 10.3 border cropping | Border cropping should be controlled so page information is not removed. | `quality_dark_border_candidate`, `quality_content_edge_cutoff_candidate`; optional processing manifest records crop and dark-border trim decisions, output size, and source hash. | Automated conservative dark-edge border screening and localized edge-content cutoff screening plus opt-in local derivative processing; original files are not modified. | Add operator approval workflow for derivative use in production packages. |
