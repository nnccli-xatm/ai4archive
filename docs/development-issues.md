...
- 主灰度 OCR 派生图默认尝试 deskew，但保留画布尺寸，不默认启用 crop、despeckle、sharpen 或超采样旋转。
...
- 新增 CLI 回归确认 `.ocr.png` 与 `ocr_binary/` 输出尺寸均等于源图，源图未修改，
  `deskew` 默认开启，`auto_crop/sharpen_text_edges` 默认关闭，guardrail failure 为 0。
- 新增斜表格页回归确认 `ocr-preprocess-leptonica-v1` 会执行 `deskew_conservative`，
  记录 `ocr_deskew_preserve_canvas`，并且纠偏后 `.ocr.png` 与 binary sidecar 不扩张画布。
- 新增 service job 回归确认 public-safe summary 不泄露路径/文件名，恢复后的
  `processing_profile` 保持 `ocr_preprocess_leptonica`。
- 2026-06-11 local-only 真实扫描样本复测：12/12 处理完成，11/12 实际纠偏并记录
  `ocr_deskew_preserve_canvas`，剩余 1 张角度低于纠偏阈值；10/12 触发
...
- 人工目检确认该批次是截至目前最佳处理结果：文字和表格线清晰度没有被旧强处理路线破坏，
  ...
  超采样扩张输出作为成功结果。
- 2026-06-11 NoisyOffice 全量复测：216/216 稳定处理，源图修改 0、输出缺失 0、失败 0、
  尺寸不匹配 0；端到端 95.173400 秒，处理阶段 55.640214 秒。整体 PSNR +2.928200 dB、
  SSIM +0.013063、MSE -766.342379、MAE -16.565967、前景保留率 -0.000002；176/216 生成
  OCR gray 和 binary，40 张经 processed output safety guard 回退，54 张需 OCR review。
