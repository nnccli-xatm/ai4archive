...
- Confirm `archive-scan-qc image-processing-capability-smoke --out
  ...
  quality baseline with tone, background-stain, fold-shadow, bleed-through,
  scanline, faded-text, illumination-gradient, ultra-pale typed-glyph, and
  text-edge aggregate deltas from synthetic fixtures, includes protected mixed
  photo/stamp/table changed-pixel, color-delta, and edge-energy drift checks
  within limits, and contains no paths, filenames, hashes, OCR text, thumbnails,
  or image content.
