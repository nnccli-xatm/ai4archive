...
- 2026-06-11 人工目检保留决策：上述 preserve-canvas Leptonica 路径是截至目前真实扫描样本
  的最佳处理结果。它保留源图尺寸，执行必须的纠偏，不再出现旧 `ocr-preprocess-v1`
  路线中的手写数字水波纹、硬吸附、超采样尺寸膨胀、笔画碎片化或表格线断裂。该路径必须作为
  后续 OCR 预处理开发的保留基线；新增算法必须在真实样本目检和聚合指标上对照它，不得回退到
  无纠偏输出、hard edge snap 或扩张画布的超采样灰度主图。已知限制是背景归一仍偏保守，
  个别低置信背景页保持灰度主图 no-op，下一步只能在不牺牲文字清晰度、纠偏和尺寸稳定性的前提下提升 OCR 可读性。
- 2026-06-11 NoisyOffice 全量复测：`ocr-preprocess-leptonica-v1` 在 216 张 simulated noisy
  grayscale 上稳定通过 CLI，源图修改 0、输出缺失 0、失败 0、尺寸不匹配 0；端到端 95.173400 秒，
  处理阶段 55.640214 秒。整体 PSNR +2.928200 dB、SSIM +0.013063、MSE -766.342379、
