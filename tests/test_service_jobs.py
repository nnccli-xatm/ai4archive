...
class ServiceJobBoundaryTests(unittest.TestCase):
    ...
    def test_service_job_runs_service_managed_custom_template_snapshot(self) -> None:
        with tempfile.TemporaryDirectory(prefix="service-job-custom-template-") as temp_dir:
            ...
            self.assertEqual(created["template"]["processing_path"], "standard-conservative-v1")
            ...
            self.assertEqual(summary["template"]["processing_path"], "standard-conservative-v1")
            ...
            self.assertEqual(record["template_snapshot"]["processing_path"], "standard-conservative-v1")
    ...
    def test_service_job_ocr_preprocess_leptonica_profile_is_public_safe(self) -> None:
        with tempfile.TemporaryDirectory(prefix="service-job-ocr-leptonica-") as temp_dir:
            root = Path(temp_dir)
            input_dir = root / "private ocr source"
            service_root = root / "service root"
            input_dir.mkdir()
            ...
            create_service_job(
                ServiceJobConfig(
                    input_dir=input_dir,
                    service_root=service_root,
                    rule_template="ocr-preprocess-leptonica-v1",
                    workers=1,
                ),
                job_id="job-ocrleptonica001",
            )

            summary = run_service_job(service_root, "job-ocrleptonica001")
            recovered = recover_service_job(service_root, "job-ocrleptonica001")
            job_root = service_root / "jobs" / "job-ocrleptonica001"
            public_raw = (job_root / SERVICE_JOB_PUBLIC_SUMMARY_JSON).read_text(encoding="utf-8")
            processing_manifest = json.loads(
                (job_root / "derivatives" / "processing_manifest.json").read_text(encoding="utf-8")
            )
            ...
            self.assertEqual(summary["state"], "finished")
            self.assertEqual(recovered["state"], "finished")
            self.assertEqual(recovered["template"]["rule_template_id"], "ocr-preprocess-leptonica-v1")
            self.assertEqual(recovered["template"]["processing_profile"], "ocr_preprocess_leptonica")
            self.assertEqual(recovered["template"]["processing_path"], "ocr-preprocess-leptonica-v1")
            self.assertEqual(processing_manifest["options"]["processing_profile"], "ocr_preprocess_leptonica")
            self.assertEqual(processing_manifest["options"]["processing_path"], "ocr-preprocess-leptonica-v1")
    ...
    def test_service_job_ocr_preprocess_opencv_local_profile_is_public_safe(self) -> None:
        with tempfile.TemporaryDirectory(prefix="service-job-ocr-opencv-local-") as temp_dir:
            root = Path(temp_dir)
            input_dir = root / "private ocr source"
            service_root = root / "service root"
            input_dir.mkdir()
            ...
            create_service_job(
                ServiceJobConfig(
                    input_dir=input_dir,
                    service_root=service_root,
                    rule_template="ocr-preprocess-opencv-local-v1",
                    workers=1,
                ),
                job_id="job-ocropencv001",
            )

            summary = run_service_job(service_root, "job-ocropencv001")
            recovered = recover_service_job(service_root, "job-ocropencv001")
            job_root = service_root / "jobs" / "job-ocropencv001"
            public_raw = (job_root / SERVICE_JOB_PUBLIC_SUMMARY_JSON).read_text(encoding="utf-8")
            processing_manifest = json.loads(
                (job_root / "derivatives" / "processing_manifest.json").read_text(encoding="utf-8")
            )
            ...
            self.assertEqual(summary["state"], "finished")
            self.assertEqual(recovered["state"], "finished")
            self.assertEqual(recovered["template"]["rule_template_id"], "ocr-preprocess-opencv-local-v1")
            self.assertEqual(recovered["template"]["processing_profile"], "ocr_preprocess_opencv_local")
            self.assertEqual(recovered["template"]["processing_path"], "ocr-preprocess-opencv-local-v1")
            self.assertEqual(processing_manifest["options"]["processing_profile"], "ocr_preprocess_opencv_local")
            self.assertEqual(processing_manifest["options"]["processing_path"], "ocr-preprocess-opencv-local-v1")
    ...
    def test_service_job_ocr_preprocess_sauvola_wolf_profile_is_public_safe(self) -> None:
        with tempfile.TemporaryDirectory(prefix="service-job-ocr-sauvola-wolf-") as temp_dir:
            root = Path(temp_dir)
            input_dir = root / "private ocr source"
            service_root = root / "service root"
            input_dir.mkdir()
            ...
            create_service_job(
                ServiceJobConfig(
                    input_dir=input_dir,
                    service_root=service_root,
                    rule_template="ocr-preprocess-sauvola-wolf-v1",
                    workers=1,
                ),
                job_id="job-ocrsauvola001",
            )

            summary = run_service_job(service_root, "job-ocrsauvola001")
            recovered = recover_service_job(service_root, "job-ocrsauvola001")
            job_root = service_root / "jobs" / "job-ocrsauvola001"
            public_raw = (job_root / SERVICE_JOB_PUBLIC_SUMMARY_JSON).read_text(encoding="utf-8")
            processing_manifest = json.loads(
                (job_root / "derivatives" / "processing_manifest.json").read_text(encoding="utf-8")
            )
            ...
            self.assertEqual(summary["state"], "finished")
            self.assertEqual(recovered["state"], "finished")
            self.assertEqual(recovered["template"]["rule_template_id"], "ocr-preprocess-sauvola-wolf-v1")
            self.assertEqual(recovered["template"]["processing_profile"], "ocr_preprocess_sauvola_wolf")
            self.assertEqual(recovered["template"]["processing_path"], "ocr-preprocess-sauvola-wolf-v1")
            self.assertEqual(processing_manifest["options"]["processing_profile"], "ocr_preprocess_sauvola_wolf")
            self.assertEqual(processing_manifest["options"]["processing_path"], "ocr-preprocess-sauvola-wolf-v1")
            self.assertEqual(processing_manifest["processing_path"]["path_id"], "ocr-preprocess-sauvola-wolf-v1")
            self.assertTrue(processing_manifest["options"]["ocr_preprocess"])
            self.assertTrue(processing_manifest["options"]["ocr_binary"])
            self.assertTrue(processing_manifest["options"]["deskew"])
            self.assertTrue(record["ocr_preprocessed"])
            ...
            self.assertIn(
                record["ocr_preprocess_reason_code"],
                {
                    "applied_sauvola_wolf_gray_baseline_normalization",
                    "applied_sauvola_wolf_fallback_gray_baseline_normalization",
                },
            )
            self.assertIn(
                record["ocr_binary_reason_code"],
                {
                    "applied_sauvola_wolf_wolf_threshold",
                    "applied_sauvola_wolf_sauvola_threshold",
                    "applied_sauvola_wolf_otsu_threshold",
                    "applied_sauvola_wolf_fallback_otsu_threshold",
                },
            )
            self.assertTrue(recovered["quality"]["provided"])
            self.assertEqual(recovered["quality"]["status"], "pass")
    ...
    def test_service_job_ocr_preprocess_structure_profile_is_public_safe(self) -> None:
        with tempfile.TemporaryDirectory(prefix="service-job-ocr-structure-") as temp_dir:
            root = Path(temp_dir)
            input_dir = root / "private ocr source"
            service_root = root / "service root"
            input_dir.mkdir()
            ...
            create_service_job(
                ServiceJobConfig(
                    input_dir=input_dir,
                    service_root=service_root,
                    rule_template="ocr-preprocess-structure-v1",
                    workers=1,
                ),
                job_id="job-ocrstructure001",
            )

            summary = run_service_job(service_root, "job-ocrstructure001")
            recovered = recover_service_job(service_root, "job-ocrstructure001")
            job_root = service_root / "jobs" / "job-ocrstructure001"
            public_raw = (job_root / SERVICE_JOB_PUBLIC_SUMMARY_JSON).read_text(encoding="utf-8")
            processing_manifest = json.loads(
                (job_root / "derivatives" / "processing_manifest.json").read_text(encoding="utf-8")
            )
            ...
            self.assertEqual(summary["state"], "finished")
            self.assertEqual(recovered["state"], "finished")
            self.assertEqual(recovered["template"]["rule_template_id"], "ocr-preprocess-structure-v1")
            self.assertEqual(recovered["template"]["processing_profile"], "ocr_preprocess_structure")
            self.assertEqual(recovered["template"]["processing_path"], "ocr-preprocess-structure-v1")
            self.assertEqual(processing_manifest["options"]["processing_profile"], "ocr_preprocess_structure")
            self.assertEqual(processing_manifest["options"]["processing_path"], "ocr-preprocess-structure-v1")
    ...
    def test_service_job_ocr_preprocess_deskew_clarity_profile_is_public_safe(self) -> None:
        with tempfile.TemporaryDirectory(prefix="service-job-ocr-deskew-clarity-") as temp_dir:
            root = Path(temp_dir)
            input_dir = root / "private ocr source"
            service_root = root / "service root"
            input_dir.mkdir()
            ...
            create_service_job(
                ServiceJobConfig(
                    input_dir=input_dir,
                    service_root=service_root,
                    rule_template="ocr-preprocess-deskew-clarity-v1",
                    workers=1,
                ),
                job_id="job-ocrdeskewclarity001",
            )

            summary = run_service_job(service_root, "job-ocrdeskewclarity001")
            recovered = recover_service_job(service_root, "job-ocrdeskewclarity001")
            job_root = service_root / "jobs" / "job-ocrdeskewclarity001"
            public_raw = (job_root / SERVICE_JOB_PUBLIC_SUMMARY_JSON).read_text(encoding="utf-8")
            processing_manifest = json.loads(
                (job_root / "derivatives" / "processing_manifest.json").read_text(encoding="utf-8")
            )
            ...
            self.assertEqual(summary["state"], "finished")
            self.assertEqual(recovered["state"], "finished")
            self.assertEqual(recovered["template"]["rule_template_id"], "ocr-preprocess-deskew-clarity-v1")
            self.assertEqual(recovered["template"]["processing_profile"], "ocr_preprocess_deskew_clarity")
            self.assertEqual(recovered["template"]["processing_path"], "ocr-preprocess-deskew-clarity-v1")
            self.assertEqual(processing_manifest["options"]["processing_profile"], "ocr_preprocess_deskew_clarity")
            self.assertEqual(processing_manifest["options"]["processing_path"], "ocr-preprocess-deskew-clarity-v1")
            ...
            self.assertIsNotNone(record["ocr_deskew_clarity_selected"])
    ...
    def test_two_async_jobs_keep_state_outputs_templates_and_reviews_isolated(self) -> None:
        with tempfile.TemporaryDirectory(prefix="service-job-concurrent-isolation-") as temp_dir:
            ...
            for job_id, spec in specs.items():
                ...
                self.assertIn("processing_path", summary["template"])
                self.assertEqual(summary["counts"]["processed_files"], 1)
                self.assertTrue(summary["quality"]["provided"])
                self.assertTrue(summary["local_review"]["provided"])
                self.assertTrue(summary["local_review"]["production_review_queue_written"])
                ...
                self.assertIn("processing_path", record["template_snapshot"])
                ...
                self.assertIn("processing_path", processing_manifest["options"])
                ...
                self.assertIn("processing_path", production_summary["operator_summary"])
