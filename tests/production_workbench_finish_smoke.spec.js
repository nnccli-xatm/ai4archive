...
test.describe("production workbench finish/export browser smoke", () => {
  ...
  test("finishes a synthetic review queue without console errors or warnings", async ({ page }) => {
      ...
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          completion_panel: {
            ...
            checklist_zh: ["打开输出文件夹，检查 3 张处理后图片的数量和画面状态", "需要重扫 0 张，需要重新处理 0 张", "确认保留原貌 1 张", "复核结果和交接说明已保存到本机状态文件夹", "准备下一批会清空当前复核队列，请重新选择新一批文件夹"],
            next_steps_zh: [
    ...
    await expect(page.locator("#completionReuseMessage")).toHaveText("本批共 3 张：已复用 2 张，实际重新处理 1 张，仍失败 0 张，剩余待处理 0 张。无需整批重跑，检查输出文件夹后交接。");
    ...
    await expect(page.getByText("确认保留原貌 1 张。")).toBeVisible();
  ...
  test("prepares a clean next batch after reviewed completion handoff", async ({ page }) => {
    let resetRequests = 0;
    ...
    await page.route("**/api/save-draft-decisions", async (route) => {
      ...
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          saved: true,
          private_path: "/tmp/private-decision-save-path",
          source_hash: "PRIVATE_DECISION_SAVE_HASH",
          ocr_snippet: "PRIVATE_DECISION_SAVE_OCR",
        }),
      });
    });
    await page.route("**/api/finish-decisions", async (route) => {
      ...
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          aggregate_only: true,
          private_path: "/tmp/private-finish-decisions-path",
          source_hash: "PRIVATE_FINISH_DECISIONS_HASH",
          ocr_snippet: "PRIVATE_FINISH_DECISIONS_OCR",
        }),
      });
    });
    ...
    await expect(page.getByText("PRIVATE_ROW_DETAIL")).toHaveCount(0);
    await expect(page.getByText("PRIVATE_STACK_TRACE")).toHaveCount(0);
    ...
    await expect(page.getByText("PRIVATE_DECISION_SAVE_HASH")).toHaveCount(0);
  ...
  test("starts a clean next batch after reviewed completion handoff", async ({ page }) => {
    ...
    await expectOperatorStatusHidesPaths(page, [
      ...
      "PRIVATE_STACK_TRACE",
      "/tmp/private-restored-status-path",
      "/tmp/private-reset-path",
      "/tmp/private-configure-path",
      "/tmp/private-start-path",
    ]);
  });

  test("shows a clean next-batch review queue after reviewed handoff start completes", async ({ page }) => {
    let resetRequests = 0;
    const configurePayloads = [];
    const startPayloads = [];
    let saveDraftRequests = 0;
    ...
    let statusPayload = {
      ...
      summary: {
        ...
        counts: { total_files: 6, processed_files: 5, failed_files: 0, retry_list_files: 0 },
      },
      progress: { schema_version: "scan-qc.production-run-progress.v1", state: "finished" },
      queue: { schema_version: "scan-qc.production-review-queue.v1", items: [] },
      restored_batch: {
      ...
      completion_panel: {
        ...
        source_path: "/tmp/private-restored-status-path/handoff.json",
        source_filename: "private-handoff.json",
        source_hash: "PRIVATE_RESTORED_HASH",
      },
    };
    await page.route("**/api/status", async (route) => {
    ...
    await page.route("**/api/save-draft-decisions", async (route) => {
      ...
      await route.fulfill({
        contentType: "application/json",
        body: JSON.stringify({ schema_version: "scan-qc.review-decision-save.v1", saved: true, aggregate_only: true }),
      });
    });
    await page.route("**/api/finish-decisions", async (route) => {
      finishDecisionRequests += 1;
      await route.fulfill({
        contentType: "application/json",
        body: JSON.stringify({ schema_version: "scan-qc.production-review-completion.v1", aggregate_only: true }),
      });
    });
    ...
    await expect(page.getByText("上一批已完成，可继续交接")).toHaveCount(0);
    await expect(page.getByText("需要继续加工时，点击准备下一批；当前复核队列会清空。")).toHaveCount(0);
    await expect(page.getByText("恢复到已完成交接面板，请先核对摘要再决定是否开始下一批。")).toHaveCount(0);
    await expect(page.getByText("已恢复上一批完成交接状态，请先核对交接摘要。")).toHaveCount(0);
    await expect(page.getByText("已处理 1 张 / 共 4 张")).toHaveCount(0);
    await expect(page.getByText("批次正在运行，请等待。")).toHaveCount(0);
    await expectOperatorStatusHidesPaths(page, [
      "/tmp/private-restored-status-path",
      "private-handoff.json",
      "PRIVATE_RESTORED_HASH",
      "/tmp/private-configure-path",
      "PRIVATE_CONFIGURE_HASH",
      "PRQ-NEXT-BATCH-1",
      "/tmp/new-next-batch-input",
      "/tmp/new-next-batch-output",
    ]);
    await expect.poll(() => saveDraftRequests).toBe(0);
    await expect.poll(() => finishDecisionRequests).toBe(0);
  });

  test("shows clean completion handoff after next-batch review decisions finish", async ({ page }) => {
    let statusPayload = {
      schema_version: "scan-qc.local-production-workbench.v1",
      running: false,
      configured: true,
    ...
    await page.route("**/api/start", async (route) => {
      statusPayload = {
        ...
        private_path: "/tmp/private-review-path",
        source_hash: "PRIVATE_REVIEW_HASH",
        ocr_snippet: "PRIVATE_REVIEW_OCR",
      };
      await route.fulfill({
        contentType: "application/json",
        body: JSON.stringify({
          ...
          running: true,
          configured: true,
          summary: { schema_version: "scan-qc.production-run.v1", status: "running", operator_summary: { message_zh: "本机正在处理图片。", total_source_images: 4, derivative_images_ready: 1, files_needing_attention: 0 } },
          progress: { schema_version: "scan-qc.production-run-progress.v1", state: "running", current_step: "quality_check", steps: [{ id: "quality_check", state: "running", completed_items: 1, total_items: 4 }] },
        }),
      });
    });
    await page.route("**/api/save-draft-decisions", async (route) => {
      await route.fulfill({ contentType: "application/json", body: JSON.stringify({ schema_version: "scan-qc.local-production-workbench.v1", saved: true }) });
    });
    await page.route("**/api/finish-decisions", async (route) => {
      const payload = JSON.parse(route.request().postData() || "{}");
      expect(payload.aggregate_counts.review_completion.complete).toBe(true);
      expect(payload.aggregate_counts.review_completion.total).toBe(1);
      expect(payload.aggregate_counts.review_completion.reviewed).toBe(1);
      ...
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          completion_panel: {
            ...
            private_path: "/tmp/private-finish-path",
            source_filename: "private-finish.json",
            source_hash: "PRIVATE_FINISH_HASH",
            ocr_snippet: "PRIVATE_FINISH_OCR",
          },
          previous_batch_retention: {
            status_zh: "上一批交接信息已保留",
            message_zh: "上一批交接摘要已保留；当前批次复核队列与文件夹设置已清空。",
    ...
    await expect(page.getByText("上一批交接信息已保留")).toHaveCount(0);
    await expectOperatorStatusHidesPaths(page, ["/tmp/private-review-path", "PRIVATE_REVIEW_HASH", "PRIVATE_REVIEW_OCR"]);
    ...
    await expect(page.locator("#completionTitle")).toHaveText("本批已完成");
    await expect(page.locator("#completionStatusFact")).toHaveText("本批已完成");
    await expect(page.locator("#completionCounts")).toHaveText("共 1 项，已确认 1 项，待决定 0 项。");
