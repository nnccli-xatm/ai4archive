...
has zero applied files in the aggregate audit counts or if the matching
public-safe quality metric maxima stay below their required evidence thresholds,
including guarded tone-normalization, small-angle deskew, paper color-cast
...
text-edge evidence from synthetic fixtures.
...
health, capabilities, create, status, cancel, and recover responses.
`archive_scan_qc.service_http` exposes those shapes through a minimal local-only
...
Service runs also compare source-image hashes before and after production
processing in memory and publish only `source_integrity` aggregate counts with
schema `scan-qc.service-job-source-integrity.v1`: checked, unchanged, modified,
missing, and added file totals. The public summary never records the hashes,
...
service-root index follows the same public-safe boundary. Start the prototype
transport with:

```powershell
archive-scan-qc service-api --service-root C:\approved\ai4archive-service-root --host 127.0.0.1 --port 8765
```
