...
def validate_completion_export_smoke(html: str, errors: list[str]) -> None:
    ...
    for required_token in [
        'await apiPost(productionApiPath("reviewActions"), reviewActionsRequest(decisionArtifact()));',
        'payload = await apiPost(productionApiPath("finishExport"), { job_id: state.jobId });',
        'payload = await apiPost(localBridgePath("finishDecisions"), decisionArtifact());',
