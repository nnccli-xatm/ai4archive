...
def process_images(
    ...
    manifest = {
        ...
        "operations": [
            ...
            _ocr_preprocess_operation_name(options) if _ocr_processing_enabled(options) else "ocr_preprocess_disabled",
...
def _output_profile_for_processing_options(options: ProcessingOptions) -> str:
    ...
    if options.processing_profile == "ocr_preprocess_leptonica":
...
def _ocr_preprocessing_operations(options: ProcessingOptions) -> list[str]:
    operations: list[str] = []
    if _ocr_processing_enabled(options):
        operations.append(_ocr_preprocess_operation_name(options))
    if options.ocr_binary:
        operations.append("ocr_binary_sidecar")
    return operations


def _ocr_preprocess_operation_name(options: ProcessingOptions) -> str:
    if options.processing_profile == "ocr_preprocess_leptonica":
    ...
    *,
    ...
    with _operation_timer(operation_timings, "ocr_preprocess", enabled=_ocr_processing_enabled(options)):
        if _ocr_processing_enabled(options):
            ...
            operations.append(
                _ocr_preprocess_operation_name(options) if ocr_preprocess.applied else "ocr_preprocess_noop"
            )
...
def _ocr_preprocess_leptonica_grayscale(
    ...
    *,
    ...
    return OcrPreprocessingResult(
        output,
        True,
        "OCR preprocessing applied: Leptonica-style background normalization with foreground preservation",
        "applied_leptonica_style_background_normalization",
        output_profile,
        round(changed_pixel_ratio, 6),
        background_before,
        background_after,
...
def _ocr_preprocess_opencv_local_grayscale(
    ...
    *,
    ...
    reason = "OCR preprocessing applied: OpenCV local background normalization with protected foreground"
    reason_code = "applied_opencv_local_background_normalization"
    result = _ocr_opencv_local_background_normalize_numpy(grayscale)
    if result is None:
        result = _ocr_leptonica_background_normalize_fallback(grayscale)
        reason = "OCR preprocessing applied: OpenCV-local fallback background normalization"
        reason_code = "applied_opencv_local_fallback_background_normalization"
    output, _changed_pixel_ratio, background_candidate_pixel_ratio = result
    changed_pixel_ratio = _pixel_change_ratio(grayscale, output)
    output_histogram = output.histogram()
    background_before = float(p95)
...
def _ocr_opencv_local_background_normalize_numpy(
    ...
    kernel_size = _odd_window_size(min(121, max(21, int(round(min_dim / 9)))))
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (kernel_size, kernel_size))
    closed = cv2.morphologyEx(source, cv2.MORPH_CLOSE, kernel)
    background = cv2.GaussianBlur(
        closed,
        (0, 0),
        sigmaX=max(1.5, kernel_size / 4.5),
        sigmaY=max(1.5, kernel_size / 4.5),
    )
    ...
    otsu_value = float(max(80.0, min(210.0, otsu_threshold)))
    block_size = _odd_adaptive_threshold_window(
        min(101, max(31, int(round(min_dim / 11)))),
        min_dim,
    )
    ...
    adaptive_foreground = adaptive <= 127
    contrast_foreground = (values <= 235.0) & (local_contrast >= 14.0)
    threshold_foreground = (values <= min(235.0, otsu_value + 28.0)) & (
        local_contrast >= 8.0
    )
    candidate_foreground = (
        contrast_foreground
        | threshold_foreground
        | (adaptive_foreground & (values <= 235.0) & (local_contrast >= 6.0))
    )
    if float(np.count_nonzero(candidate_foreground)) / max(1, source.size) > 0.35:
        candidate_foreground = candidate_foreground & (values <= max(170.0, otsu_value + 8.0))
    strong_foreground = (values <= min(170.0, otsu_value)) | (
        candidate_foreground & (local_contrast >= 18.0)
    )
    ...
    protected = cv2.dilate(
        foreground_seed.astype(np.uint8),
        np.ones((3, 3), dtype=np.uint8),
        iterations=1,
    ).astype(bool)
    output = values.copy()
    background_mask = ~protected
    lift = np.clip(248.0 - background_values, 0.0, 72.0)
    leveled = values + lift
    whitened = 255.0 - ((255.0 - leveled) * 0.16)
...
def _ocr_preprocess_structure_grayscale(
    image: Image.Image,
    *,
    force_grayscale: bool = False,
) -> OcrPreprocessingResult:
    ...
    result = _ocr_structure_background_normalize_numpy(grayscale)
    reason = "OCR preprocessing applied: structure-preserving stroke and table-line enhancement"
    reason_code = "applied_structure_preserving_background_normalization"
    if result is None:
        result = _ocr_stroke_bg_background_normalize_numpy(grayscale)
        reason = "OCR preprocessing applied: structure-preserving fallback background normalization"
        reason_code = "applied_structure_preserving_fallback_background_normalization"
    if result is None:
        result = _ocr_leptonica_background_normalize_fallback(grayscale)
        reason = "OCR preprocessing applied: structure-preserving grayscale fallback"
        reason_code = "applied_structure_preserving_grayscale_fallback"
    output, _changed_pixel_ratio, background_candidate_pixel_ratio = result
    changed_pixel_ratio = _pixel_change_ratio(grayscale, output)
    output_histogram = output.histogram()
    background_before = float(p95)
...
def _ocr_structure_background_normalize_numpy(
    ...
    height, width = source.shape
    min_dim = max(1, min(width, height))
    kernel_size = _odd_window_size(min(171, max(31, int(round(min_dim / 7)))))
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (kernel_size, kernel_size))
    closed = cv2.morphologyEx(source, cv2.MORPH_CLOSE, kernel)
    background = cv2.GaussianBlur(
        closed,
        (0, 0),
        sigmaX=max(2.0, kernel_size / 4.0),
        sigmaY=max(2.0, kernel_size / 4.0),
    )
    values = source.astype(np.float32)
    background_values = np.maximum(background.astype(np.float32), values + 1.0)
    local_contrast = background_values - values
    blurred = cv2.GaussianBlur(source, (3, 3), 0)
    otsu_threshold, _otsu = cv2.threshold(blurred, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    otsu_value = float(max(70.0, min(225.0, otsu_threshold)))

    sobel_x = cv2.Sobel(blurred, cv2.CV_32F, 1, 0, ksize=3)
    sobel_y = cv2.Sobel(blurred, cv2.CV_32F, 0, 1, ksize=3)
    gradient = cv2.magnitude(sobel_x, sobel_y)
    gradient_reference = float(np.percentile(gradient, 95.0))
    gradient_threshold = max(8.0, min(38.0, gradient_reference * 0.32))
    gradient_high = max(16.0, min(64.0, gradient_reference * 0.50))

    dark_line_candidate = (
        (values <= min(238.0, otsu_value + 42.0))
        & ((local_contrast >= 3.0) | (gradient >= gradient_threshold))
    )
    line_seed = np.zeros(source.shape, dtype=np.uint8)
    horizontal_lengths = {
        max(9, min(221, int(round(width / 18)))),
        max(9, min(161, int(round(width / 38)))),
    }
    vertical_lengths = {
        max(9, min(221, int(round(height / 18)))),
        max(9, min(161, int(round(height / 38)))),
    }
    candidate_u8 = dark_line_candidate.astype(np.uint8) * 255
    for horizontal_width in horizontal_lengths:
        horizontal_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (horizontal_width, 1))
        line_seed = cv2.bitwise_or(line_seed, cv2.morphologyEx(candidate_u8, cv2.MORPH_OPEN, horizontal_kernel))
    for vertical_height in vertical_lengths:
        vertical_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (1, vertical_height))
        line_seed = cv2.bitwise_or(line_seed, cv2.morphologyEx(candidate_u8, cv2.MORPH_OPEN, vertical_kernel))
    line_mask = line_seed > 0
    if float(np.count_nonzero(line_mask)) / max(1, source.size) > 0.22:
        line_mask = line_mask & (
            (values <= max(170.0, otsu_value + 8.0))
            | ((local_contrast >= 16.0) & (gradient >= gradient_threshold))
        )

    contrast_foreground = (values <= 245.0) & (local_contrast >= 14.0)
    threshold_foreground = (values <= min(238.0, otsu_value + 34.0)) & (local_contrast >= 5.0)
    gradient_foreground = (values <= 224.0) & (gradient >= gradient_threshold)
    candidate_foreground = contrast_foreground | threshold_foreground | gradient_foreground | line_mask
    if float(np.count_nonzero(candidate_foreground)) / max(1, source.size) > 0.36:
        candidate_foreground = candidate_foreground & (
            (values <= max(175.0, otsu_value + 10.0))
            | (local_contrast >= 20.0)
            | line_mask
        )
    strong_foreground = (
        (values <= min(168.0, otsu_value + 5.0))
        | ((values <= 226.0) & (local_contrast >= 22.0))
        | ((values <= 195.0) & (gradient >= gradient_high))
        | line_mask
    )
    foreground_seed = _ocr_leptonica_foreground_component_mask(
        np,
    ...
    line_protected = cv2.dilate(
        line_mask.astype(np.uint8),
        cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3)),
        iterations=1,
    ).astype(bool)
    stroke_protected = cv2.dilate(
        foreground_seed.astype(np.uint8),
        np.ones((3, 3), dtype=np.uint8),
        iterations=1,
    ).astype(bool)
    protected = stroke_protected | line_protected

    output = values.copy()
    background_mask = ~protected
    lift = np.clip(250.0 - background_values, 0.0, 70.0)
    leveled = values + lift
    whitened = 255.0 - ((255.0 - leveled) * 0.14)
    output[background_mask] = np.maximum(values[background_mask], whitened[background_mask])

    detail_blur = cv2.GaussianBlur(values, (0, 0), sigmaX=0.8, sigmaY=0.8)
    dark_detail = np.minimum(values - detail_blur, 0.0)
    stroke_boost = np.clip((-dark_detail * 0.38) + np.clip(local_contrast * 0.045, 0.0, 5.0), 0.0, 12.0)
    stroke_values = np.maximum(0.0, values - stroke_boost)
    structure_mask = protected & (values <= 235.0)
    output[structure_mask] = np.minimum(output[structure_mask], stroke_values[structure_mask])
    output[line_protected & (values <= 235.0)] = np.minimum(
        output[line_protected & (values <= 235.0)],
        np.maximum(0.0, values[line_protected & (values <= 235.0)] - 3.0),
    )
...
def _ocr_leptonica_background_normalize_numpy(
    ...
    height, width = source.shape
    min_dim = max(1, min(width, height))
    kernel_size = _odd_window_size(min(101, max(15, int(round(min_dim / 10)))))
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (kernel_size, kernel_size))
    closed = cv2.morphologyEx(source, cv2.MORPH_CLOSE, kernel)
    background = cv2.GaussianBlur(
        closed,
        (0, 0),
        sigmaX=max(1.5, kernel_size / 5.0),
        sigmaY=max(1.5, kernel_size / 5.0),
    )
    values = source.astype(np.float32)
    background_values = np.maximum(background.astype(np.float32), values + 1.0)
    local_contrast = background_values - values
    contrast_foreground = (values <= 235.0) & (local_contrast >= 18.0)
    if float(np.count_nonzero(contrast_foreground)) / max(1, source.size) > 0.35:
        contrast_foreground = contrast_foreground & (values <= 170.0)
    strong_foreground = (values <= 150.0) | (contrast_foreground & (local_contrast >= 34.0))
    foreground_seed = _ocr_leptonica_foreground_component_mask(
        np,
        cv2,
        strong_foreground=strong_foreground,
        candidate_foreground=strong_foreground | contrast_foreground,
    )
    protected = cv2.dilate(
        foreground_seed.astype(np.uint8),
        np.ones((3, 3), dtype=np.uint8),
    ...
    output = values.copy()
    background_mask = ~protected
    lifted = values + np.clip(250.0 - background_values, 0.0, 70.0)
    whitened = 255.0 - ((255.0 - lifted) * 0.14)
    output[background_mask] = np.maximum(values[background_mask], whitened[background_mask])
    output[protected] = values[protected]
    output_u8 = np.clip(np.rint(output), 0, 255).astype(np.uint8)
    changed = np.abs(output_u8.astype(np.int16) - source.astype(np.int16)) > 8
...
def _ocr_leptonica_foreground_component_mask(
    np: Any,
    cv2: Any,
    *,
    strong_foreground: Any,
    candidate_foreground: Any,
) -> Any:
    seed = candidate_foreground.astype(np.uint8)
    label_count, labels, stats, _centroids = cv2.connectedComponentsWithStats(seed, connectivity=8)
    if label_count <= 1:
        return candidate_foreground & False
    strong_counts = np.bincount(labels[strong_foreground].ravel(), minlength=label_count)
    keep_labels = np.zeros(label_count, dtype=bool)
    for label in range(1, label_count):
        area = int(stats[label, cv2.CC_STAT_AREA])
        width = int(stats[label, cv2.CC_STAT_WIDTH])
        height = int(stats[label, cv2.CC_STAT_HEIGHT])
        strong_overlap = bool(strong_counts[label])
        if area >= 4 or max(width, height) >= 4 or (strong_overlap and area >= 2):
            keep_labels[label] = True
    return keep_labels[labels]


def _ocr_leptonica_background_normalize_fallback(
    grayscale: Image.Image,
) -> tuple[Image.Image, float, float]:
    width, height = grayscale.size
    kernel_size = _odd_window_size(min(101, max(15, int(round(min(width, height) / 10)))))
    background = grayscale.filter(ImageFilter.MaxFilter(kernel_size)).filter(
        ImageFilter.GaussianBlur(radius=max(1.5, kernel_size / 5.0))
    )
    ...
    for y in range(height):
        for x in range(width):
            ...
            background_value = max(value + 1, int(background_pixels[x, y]))
            if value <= 235 and background_value - value >= 18:
                contrast_pixels += 1
    dense_contrast = contrast_pixels / total > 0.35
    ...
    for y in range(height):
        for x in range(width):
            value = int(source_pixels[x, y])
            background_value = max(value + 1, int(background_pixels[x, y]))
            local_contrast = background_value - value
            contrast_protected = value <= 235 and local_contrast >= 18
            if dense_contrast:
                contrast_protected = contrast_protected and value <= 170
            protected = value <= 150 or contrast_protected
            if protected:
                output_value = value
            else:
                lifted = value + max(0, min(70, 250 - background_value))
                output_value = max(value, int(round(255 - ((255 - lifted) * 0.14))))
            output_pixels[x, y] = max(0, min(255, output_value))
            if abs(output_pixels[x, y] - value) > 8:
                changed += 1
                if not protected:
                    background_changed += 1
    return output, changed / total, background_changed / total


def _odd_window_size(value: int) -> int:
...
def _ocr_leptonica_binary_sidecar(
    image: Image.Image,
    ocr_preprocess: OcrPreprocessingResult,
) -> OcrBinaryResult:
    grayscale = image.convert("L")
    result = _ocr_leptonica_binary_numpy(grayscale)
    if result is None:
        threshold = _otsu_threshold(grayscale.histogram(), max(1, grayscale.width * grayscale.height))
        threshold = max(80, min(235, threshold))
        binary = grayscale.point(lambda value: 0 if value <= threshold else 255, mode="L")
        reason = "OCR binary sidecar applied: Leptonica-style Otsu threshold on protected grayscale derivative"
        reason_code = "applied_leptonica_style_otsu_threshold"
    else:
...
def _ocr_opencv_local_binary_sidecar(
    image: Image.Image,
    ocr_preprocess: OcrPreprocessingResult,
) -> OcrBinaryResult:
    grayscale = image.convert("L")
    result = _ocr_opencv_local_binary_numpy(grayscale)
    if result is None:
        threshold = _otsu_threshold(grayscale.histogram(), max(1, grayscale.width * grayscale.height))
        threshold = max(80, min(235, threshold))
        binary = grayscale.point(lambda value: 0 if value <= threshold else 255, mode="L")
        reason = "OCR binary sidecar applied: OpenCV-local fallback Otsu threshold"
        reason_code = "applied_opencv_local_otsu_threshold"
    else:
        binary, threshold, reason, reason_code = result
    foreground_ratio = _dark_pixel_ratio(binary, 127)
    source_dark = grayscale.point(lambda value: 255 if value <= 127 else 0, mode="L")
...
def _ocr_opencv_local_binary_numpy(
    grayscale: Image.Image,
    ...
    block_size = _odd_adaptive_threshold_window(min(101, max(31, int(round(min_dim / 11)))), min_dim)
    ...
    adaptive_foreground_ratio = float(np.count_nonzero(adaptive_binary <= 127)) / max(1, source.size)
    if 0.001 <= adaptive_foreground_ratio <= 0.45:
        return (
            Image.fromarray(adaptive_binary.astype(np.uint8), mode="L"),
            None,
            "OCR binary sidecar applied: OpenCV-local adaptive threshold",
...
def _ocr_structure_binary_sidecar(
    image: Image.Image,
    ocr_preprocess: OcrPreprocessingResult,
) -> OcrBinaryResult:
    grayscale = image.convert("L")
    result = _ocr_structure_binary_numpy(grayscale)
    if result is None:
        threshold = _otsu_threshold(grayscale.histogram(), max(1, grayscale.width * grayscale.height))
        threshold = max(80, min(235, threshold))
        binary = grayscale.point(lambda value: 0 if value <= threshold else 255, mode="L")
        reason = "OCR binary sidecar applied: structure-preserving fallback Otsu threshold"
        reason_code = "applied_structure_fallback_otsu_threshold"
    else:
        binary, threshold, reason, reason_code = result
    foreground_ratio = _dark_pixel_ratio(binary, 127)
    source_dark = grayscale.point(lambda value: 255 if value <= 127 else 0, mode="L")
...
def _ocr_structure_binary_numpy(
    grayscale: Image.Image,
) -> tuple[Image.Image, int | None, str, str] | None:
    np = _load_numpy()
    cv2 = _load_opencv()
    ...
    height, width = source.shape
    min_dim = max(3, min(height, width))
    sobel_x = cv2.Sobel(blurred, cv2.CV_32F, 1, 0, ksize=3)
    sobel_y = cv2.Sobel(blurred, cv2.CV_32F, 0, 1, ksize=3)
    gradient = cv2.magnitude(sobel_x, sobel_y)
    gradient_reference = float(np.percentile(gradient, 95.0))
    gradient_threshold = max(8.0, min(40.0, gradient_reference * 0.32))
    line_candidate = ((source <= min(238, threshold + 42)) & (gradient >= gradient_threshold)).astype(np.uint8) * 255
    line_seed = np.zeros(source.shape, dtype=np.uint8)
    for horizontal_width in {
        max(9, min(221, int(round(width / 18)))),
        max(9, min(161, int(round(width / 38)))),
    }:
        horizontal_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (horizontal_width, 1))
        line_seed = cv2.bitwise_or(line_seed, cv2.morphologyEx(line_candidate, cv2.MORPH_OPEN, horizontal_kernel))
    for vertical_height in {
        max(9, min(221, int(round(height / 18)))),
        max(9, min(161, int(round(height / 38)))),
    }:
        vertical_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (1, vertical_height))
        line_seed = cv2.bitwise_or(line_seed, cv2.morphologyEx(line_candidate, cv2.MORPH_OPEN, vertical_kernel))
    line_mask = cv2.dilate(
        (line_seed > 0).astype(np.uint8),
        cv2.getStructuringElement(cv2.MORPH_RECT, (3, 3)),
        iterations=1,
    ).astype(bool)
    ...
    foreground_ratio = float(np.count_nonzero(otsu_binary <= 127)) / max(1, source.size)
    if 0.001 <= foreground_ratio <= 0.45:
        binary = otsu_binary.copy()
        binary[line_mask] = 0
        return (
            Image.fromarray(binary.astype(np.uint8), mode="L"),
            threshold,
            "OCR binary sidecar applied: structure-preserving Otsu threshold",
    ...
    adaptive_binary[line_mask] = 0
    return (
        Image.fromarray(adaptive_binary.astype(np.uint8), mode="L"),
        None,
        "OCR binary sidecar applied: structure-preserving adaptive threshold",
...
def _ocr_leptonica_binary_numpy(
    ...
    if source.ndim != 2:
        return None
    blurred = cv2.GaussianBlur(source, (3, 3), 0)
    threshold_value, binary_array = cv2.threshold(blurred, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    threshold: int | None = int(round(float(threshold_value)))
    reason = "OCR binary sidecar applied: Leptonica-style Otsu threshold on protected grayscale derivative"
    reason_code = "applied_leptonica_style_otsu_threshold"
    foreground_ratio = float(np.count_nonzero(binary_array <= 127)) / max(1, source.size)
    if foreground_ratio < 0.001 or foreground_ratio > 0.45:
        min_dim = max(3, min(source.shape))
        block_size = _odd_adaptive_threshold_window(min(101, max(31, int(round(min_dim / 12)))), min_dim)
        ...
        threshold = None
        reason = "OCR binary sidecar applied: Leptonica-style adaptive threshold on protected grayscale derivative"
        reason_code = "applied_leptonica_style_adaptive_threshold"
    return Image.fromarray(binary_array.astype(np.uint8), mode="L"), threshold, reason, reason_code


def _odd_adaptive_threshold_window(value: int, min_dim: int) -> int:
    upper = max(3, min_dim if min_dim % 2 == 1 else min_dim - 1)
    window = min(_odd_window_size(value), upper)
    if window % 2 == 0:
        window -= 1
    return max(3, window)
...
def _lighten_fold_shadows_conservative(image: Image.Image) -> FoldShadowCleanupResult:
    ...
    curved_vertical = _fold_shadow_curved_axis_plan(grayscale, protected, vertical=True, background=p95)
    curved_horizontal = _fold_shadow_curved_axis_plan(grayscale, protected, vertical=False, background=p95)
    ...
    plan = max(
        (vertical, horizontal, curved_vertical, curved_horizontal, diagonal_tl_br, diagonal_tr_bl),
...
def _fold_shadow_curved_axis_plan(
    grayscale: Image.Image,
    protected: Image.Image,
    *,
    vertical: bool,
    ...
    axis_length = width if vertical else height
    cross_length = height if vertical else width
    orientation = "curved_vertical" if vertical else "curved_horizontal"
    edge_margin = max(12, int(round(axis_length * 0.08)))
    cross_margin = max(10, int(round(cross_length * 0.05)))
    if axis_length <= edge_margin * 2 or cross_length <= cross_margin * 2:
        return _empty_fold_shadow_plan(orientation, "image too small")

    pixels = grayscale.load()
    protected_pixels = protected.load()
    min_width = max(2, int(round(axis_length * 0.006)))
    max_width = max(min_width, int(round(axis_length * 0.045)))
    neighbor_radius = max(max_width * 3, min_width + 4)
    cross_candidates: list[dict[str, Any]] = []
    all_candidates: set[tuple[int, int]] = set()

    for cross in range(cross_margin, cross_length - cross_margin):
        runs: list[list[int]] = []
        current_run: list[int] = []
        for index in range(edge_margin, axis_length - edge_margin):
            x, y = (index, cross) if vertical else (cross, index)
            value = int(pixels[x, y])
            if not protected_pixels[x, y] and 3 <= background - value <= 24 and value >= 188:
        ...
        for run in runs:
            ...
            for offset in range(-neighbor_radius, neighbor_radius + 1):
                if abs(offset) < band_width + 2:
                    continue
                neighbor = center + offset
                if not (edge_margin <= neighbor < axis_length - edge_margin):
                    continue
                nx, ny = (neighbor, cross) if vertical else (cross, neighbor)
                if protected_pixels[nx, ny]:
                    continue
                neighbor_values.append(int(pixels[nx, ny]))
            ...
            if not (2.25 <= local_delta <= 42.0):
                continue
            selected = {(index, cross) if vertical else (cross, index) for index in run}
            score = local_delta * min(1.0, band_width / max(1, min_width))
        ...
        if best is not None:
            ...
            all_candidates.update(best["selected"])
    ...
    groups = _contiguous_groups([int(candidate["cross"]) for candidate in cross_candidates])
    by_cross = {int(candidate["cross"]): candidate for candidate in cross_candidates}
    selected_groups: list[list[int]] = []
    selected: set[tuple[int, int]] = set()
    score = 0.0
    for group in groups:
        if len(group) < max(36, int(round(cross_length * 0.42))):
            continue
        continuity = _fold_shadow_cross_continuity(set(group), cross_length)
        if not continuity["usable"]:
            continue
        centers = [int(by_cross[cross]["center"]) for cross in group]
        ...
        residuals = [
            abs(
                ...
                - (
                    centers[0]
                    + (centers[-1] - centers[0]) * ((cross - first_cross) / max(1, last_cross - first_cross))
                )
            )
            for cross, center in zip(group, centers)
        ]
        if max(residuals) < 2.5 or (sum(residuals) / len(residuals)) < 1.2:
            continue
        max_step = max(abs(current - previous) for previous, current in zip(centers, centers[1:]))
        if max_step > max(4, int(round(axis_length * 0.025))):
            continue
        protected_corridor_crosses = 0
        dark_corridor_crosses = 0
        for cross, center in zip(group, centers):
            corridor_half_width = max((max_width // 2) + 4, 7)
            has_protected = False
            has_dark_content = False
            for index in range(center - corridor_half_width, center + corridor_half_width + 1):
                if not (0 <= index < axis_length):
                    continue
                x, y = (index, cross) if vertical else (cross, index)
                if protected_pixels[x, y]:
                    has_protected = True
                if int(pixels[x, y]) <= background - 26:
                    has_dark_content = True
            if has_protected:
                protected_corridor_crosses += 1
            if has_dark_content:
                dark_corridor_crosses += 1
        if protected_corridor_crosses >= max(3, int(round(len(group) * 0.018))):
            return _empty_fold_shadow_plan(
                orientation,
                "foreground intersects candidate fold band",
                candidate_total_ratio,
            )
        if dark_corridor_crosses >= max(3, int(round(len(group) * 0.018))):
            return _empty_fold_shadow_plan(
                orientation,
                "foreground intersects candidate fold band",
                candidate_total_ratio,
            )
        group_selected: set[tuple[int, int]] = set()
        local_deltas: list[float] = []
        for cross in group:
            candidate = by_cross[cross]
            group_selected.update(candidate["selected"])
            local_deltas.append(float(candidate["local_delta"]))
        selected_groups.append(group)
        selected.update(group_selected)
        score += min(1.0, sum(local_deltas) / max(1, len(local_deltas)) / 12.0) * min(
            1.0,
            len(group) / max(1, int(round(cross_length * 0.70))),
        )
    ...
    max_selected_width = max(int(by_cross[cross]["band_width"]) for group in selected_groups for cross in group)
    return {
        "orientation": orientation,
        "score": score,
        "bands": selected_groups,
        "selected": selected,
        "candidate_ratio": len(selected) / max(1, width * height),
        "width_bucket": _fold_shadow_width_bucket(max_selected_width),
        "coverage_bucket": _fold_shadow_coverage_bucket(len(selected) / max(1, width * height)),
        "reason": None,
    }
...
def _enhance_faded_text_conservative(
    ...
    *,
    ...
    threshold = min(232, p50 - 8, p95 - 10)
    ultra_faint_threshold = threshold > 230 and p01 >= 230
    if threshold < 125:
    ...
    if ultra_faint_threshold and (text_like_components < 12 or selected_ratio > 0.04):
        return _faded_text_noop(image, "faded text enhancement skipped: text evidence too weak", candidate_ratio)
...
def _despeckle_edge_tiny_isolated_component_for_cleanup(
    ...
    if not candidate_points or len(candidate_points) > _DESPECKLE_EDGE_TINY_ISOLATED_MAX_PIXELS:
        return None
    components, _component_by_point = _despeckle_candidate_components(candidate_points)
    if len(components) != 1 or len(components[0]) > _DESPECKLE_EDGE_TINY_ISOLATED_MAX_PIXELS:
        return None

    component = components[0]
    component_x = [point[0] for point in component]
    component_y = [point[1] for point in component]
    if max(component_x) - min(component_x) + 1 > 2 or max(component_y) - min(component_y) + 1 > 2:
        return None
    ...
    candidate_set = set(candidate_points)
    for x, y in component:
        if _despeckle_pixel_color_protected(_despeckle_pixel_at(rgb_pixels, x, y)):
            return None
        if _despeckle_nearby_content_context_count(gray_pixels, width, height, x, y, stop_at=1):
            return None
        if _despeckle_has_sparse_text_protected_context(
            gray_pixels,
            rgb_pixels,
